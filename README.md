...
An often misunderstood piece of Python import machinery is the __all__ attribute. While it is completely optional, it’s common to see modules with the __all__ list populated explicitly. Before explaining what the real purpose of __all__ is (and how it relates to the titular wild imports), let’s deconstruct some common misconceptions by highlighting what it isn’t: 1. __all__ doesn’t prevent any of the module symbols (functions, classes, etc.) from being directly imported; 2. Similarly, __all__ doesn’t influence what symbols are included in the results of dir(module) or vars(module). In other words, the content of __all__ is more of a convention rather than a strict limitation. Regardless of what you put there, every symbol defined in your module will still be accessible from the outside. So, what does __all__ actually effect? Simply speaking, its purpose is twofold: 1. It tells the readers of the source code — be it humans or automated tools — what’s the conventional public API exposed by the module; 2. It lists names to import when performing the so-called wild import: from module import *. In Python, like in many other languages, it is recommended to be explicit about the exact functions and classes we’re importing. 
